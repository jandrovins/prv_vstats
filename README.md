...
```
python prvvstats.py <trace.prv> [--event-type N] [--filter-at-parse] [--progress] [--output FILE] [--llm-output FILE]
```
...
| `--progress` | off | Show live loading progress for `.prv` (percent, ETA, throughput, counts, memory) |
